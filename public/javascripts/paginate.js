function paginate(pageNumber, pages) {
    ...
    switch(true)
    {
        case (pages > 5 && pageNumber < 5) :{
            ...
            pagination += dots;
            pagination += inActiveList(pages);break;
        }
        case (pages <= 5 && pageNumber <= 5) : pagination +=reducedFromIterator(1,pages,pageNumber,0); break;
        case (pages > 5 && pageNumber >= 5 && pages == pageNumber) : {
            pagination += reverseIterator(5, 1, pageNumber, pages);
            pagination += dots;
            pagination += pageNumber == pages ? activeList(pages) : inActiveList(pages);break;
        }
