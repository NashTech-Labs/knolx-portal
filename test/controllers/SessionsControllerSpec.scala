...
class SessionsControllerSpec extends PlaySpecification with Results {
  ...
  private val sessionObject =
    Future.successful(List(SessionInfo(_id.stringify, "email", BSONDateTime(date.getTime), "sessions", "feedbackFormId", "topic",
      1, meetup = true, "rating", cancelled = false, active = true, BSONDateTime(date.getTime), Some("youtubeURL"), Some("slideShareLink"), reminder = false, notification = false, _id)))
  ...
  "Session Controller" should {
    ...
    "render getByEmail session form" in new WithTestApplication {
      ...
      val sessionInfo = Future.successful(Some(SessionInfo(_id.stringify, "test@knoldus.com", BSONDateTime(date.getTime), "session 1",
        "feedbackFormId", "topic", 1, meetup = false, "", cancelled = false, active = true, BSONDateTime(date.getTime), Some("youtubeURL"), Some("slideShareLink"), reminder = false, notification = false, _id)))
    ...
    "getByEmail session" in new WithTestApplication {
      ...
      val updatedInformation = UpdateSessionInfo(UpdateSessionInformation(_id.stringify, date, "session 1",
        "feedbackFormId", "topic", 1, Some("youtubeURL"), Some("slideShareLink"), meetup = true), BSONDateTime(1498415399000L))
      val updateWriteResult = Future.successful(UpdateWriteResult(ok = true, 1, 1, Seq(), Seq(), None, None, None))
